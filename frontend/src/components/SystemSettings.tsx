...
export default function SystemSettings({en,value,onChange}:{en:boolean;value:Preferences;onChange:(value:Preferences)=>void}) {
  ...
  return <div className="grid ai-grid"><AIConnection en={en}/><div><section className="card"><h2>{t("任务与报告", "Jobs & reports")}</h2>
    ...
    <label className="check"><input name="pdf" type="checkbox" defaultChecked={value.default_pdf}/>{t("默认额外生成 PDF", "Generate PDF by default")}</label><p className="muted">{t("PDF 需要系统依赖，HTML 报告始终保留。", "PDF requires system libraries; HTML reports remain available.")}</p>
