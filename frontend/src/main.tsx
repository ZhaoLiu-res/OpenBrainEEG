...
type Job = { id: string; created_at?: string; config?: unknown; steps?: unknown; filename: string; status: string; progress: number; step: string; warnings: string[]; error: string | null; outputs: Record<string,string>; metrics?: Record<string,unknown>; dataset: { n_channels: number; duration: number; sfreq: number } };
...
function App() {
  ...
  const names: Record<string,string> = {settings:t("系统设置","System settings"),home:t("首页","Home"),samples:t("样例运行","Samples"),clean:t("数据清洗","EEG cleaning"),history:t("历史任务与报告","History & reports"),ai:t("AI 报告助手","AI assistant"),jobs:t("任务进度","Job progress"),reports:t("报告查看","Report viewer")};
  ...
  const downloads = (j: Job) => <div className="downloads">{Object.keys(j.outputs).map(kind => <a key={kind} href={`/api/jobs/${j.id}/files/${kind}`}>{kind === "html" ? t("下载 HTML 报告","Download HTML report") : kind === "parameters" ? t("处理参数 JSON","Parameters JSON") : kind.toUpperCase()} ↓</a>)}</div>;
  ...
  return <div className="shell">
    ...
    <aside><a className="brand" href="#/home"><img src="/brainifly-icon-wave.svg" alt="" width="36" height="36"/><span>Brainifly<small>{t("脑电研究工作台", "EEG research workspace")}</small></span></a>
      <nav aria-label={t("主导航","Main navigation")}>{["home","samples","clean","history","ai","settings"].map(page=><a key={page} href={`#/${page}`} aria-current={tab===page ? "page" : undefined} className={tab===page || (page==="history" && ["jobs","reports"].includes(tab)) ? "active" : ""}>{names[page]}</a>)}</nav>
    ...
    <main id="main-content" tabIndex={-1}><header><span className="eyebrow">{names[tab] || "Brainifly"}</span><label className="language-picker"><span>{t("语言", "Language")}</span><select aria-label={t("界面语言", "Interface language")} value={languageMode} onChange={e=>changeLanguageMode(e.target.value as "auto"|"zh"|"en")}><option value="auto">{t("自动 / IP", "Auto / IP")}</option><option value="zh">中文</option><option value="en">English</option></select></label></header>
      ...
      {tab !== "home" && <div className="page-heading"><h1>{names[tab] || t("页面不存在","Page not found")}</h1><p className="subtitle">{t("从样例到自己的数据，从处理进度到完整报告。","From samples to your recordings, from processing to full reports.")}</p></div>}
      {tab === "home" && <>
      ...
      {tab === "clean" && <>        <section className="grid"><div className="card"><div className="section-title"><span className="number">01</span><h2>{t("导入记录", "Import recording")}</h2></div>
          ...
          <label className="check"><input type="checkbox" checked={pdf} onChange={e => setPdf(e.target.checked)}/>{t("额外导出 PDF（需安装系统库）", "Also export PDF (system libraries required)")}</label>
      ...
      {["jobs","reports"].includes(tab) && (!current ? <div className="card">{loaded?t("未找到此任务，请返回历史记录选择。","Job not found. Select a job from history."):t("正在读取任务…","Loading job…")}<button className="secondary" onClick={()=>setTab("history")}>{names.history}</button></div> : <>
        ...
        {downloads(current)}{current.status === "completed" && tab === "jobs" && <a className="primary" href={`#/reports/${current.id}`}>{t("查看完整报告", "View full report")}</a>}{current.status === "completed" && <button className="secondary" onClick={()=>{setSelected(current.id);setTab("ai");}}>{t("向 AI 提问","Ask AI")} →</button>}</section>
